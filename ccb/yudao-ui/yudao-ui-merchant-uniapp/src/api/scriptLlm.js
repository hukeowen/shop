...
`.trim();
  ...
  const structureHint =
    SCENES <= 2 ? '① 3 秒强钩子（抛痛点/反差/疑问停住手指）② 亮点证据+扫码下单指令' :
    SCENES === 3 ? '① 3 秒钩子（痛点/反差/疑问，让人停刷）② 真材实料的证据/过程/细节（让人心动）③ 成品高光+稀缺/限时+扫右下角二维码下单' :
    `① 钩子（抛痛点/反差）② 真材实料的证据/过程 ${SCENES > 4 ? '③④ 多角度细节+对比' : '③ 细节高潮'} ④ 情感共鸣/口碑背书 ⑤ 稀缺限时+扫右下角二维码下单（共 ${SCENES} 幕，层层递进）`;
  ...
  const directorPrompt = [
    ...
    '5. 【收尾行动号召】最后一幕必须有稀缺/限时/低价话术 + 明确扫码指令（"扫屏幕右下角二维码"/"长按识别右下角二维码"/"扫码直接拍，库存不多手慢无"）；绝对不要说"小黄车""购物车""点击链接"',
