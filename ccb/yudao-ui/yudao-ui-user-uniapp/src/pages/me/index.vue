<template>
  <view class="page">
      ...
      <view class="poster-wrap" @click.stop>
        <view class="poster" id="invite-poster">
          ...
          <view v-if="posterRule" class="poster-rule">
            <view class="poster-rule-title">📖 邀 {{ posterRule.n }} 累积 规则</view>
    ...
    <view class="me-grid">
      ...
      <view class="me-row" @click="goQueue"><view class="me-row-icon alt-2">🔥</view><text class="me-row-name">我的队列（邀 N 累积）</text><text v-if="queueTotal" class="me-row-tag">{{ queueTotal }} 个</text><text class="me-row-arrow">›</text></view>
...
function onShopInvite(s) {
  if (!user.isLogin) return goLogin();
  if (!s.inviteEligible) {
    uni.showToast({ title: '先在本店完成「邀 N 累积」购买才能开启邀请', icon: 'none', duration: 2200 });
    return;
  }
  // 构造海报数据 + QR 链接
  ...
  try {
    ...
    myShops.value = (list || []).map((s) => {
      ...
      const queueBar = (queue && reqN > 0) ? {
        text: `${queue.spuName || '商品'} · 邀 ${reqN} 累积进度`,
      ...
      return {
        ...
        // 邀请门槛：购买过邀 N 累积 商品（queueing OR completed 均算）
        inviteEligible: !!elig,
