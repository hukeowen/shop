<template>
  <view class="page">
    <nav-bar title="分类" />
    <view class="layout">
      <scroll-view scroll-y class="side">
        <view v-for="c in cats" :key="c.k" class="side-item" :class="{ on: active === c.k }" @click="active = c.k">
          {{ c.label }}
        </view>
      </scroll-view>
      <scroll-view scroll-y class="main">
        <view v-if="!products.length" class="empty-wrap">
          <empty-state title="该分类暂无商品" />
        </view>
        <view v-else>
          <view v-for="p in products" :key="p.id" class="prod" @click="goProduct(p)">
            <view class="prod-pic">{{ p.em || '🛍' }}</view>
            <view class="prod-body">
              <view class="prod-name">{{ p.name }}</view>
              <view class="prod-price">¥{{ p.price }}</view>
            </view>
          </view>
        </view>
      </scroll-view>
    </view>
  </view>
</template>

<script setup>
import { ref, watch, onMounted } from 'vue';
// import { listCategories, listByCategory } from '@/api/product.js';

const cats = ref([
  { k: 'food', label: '餐饮' },
  { k: 'tea',  label: '茶饮' },
  { k: 'bake', label: '烘焙' },
  { k: 'fresh',label: '生鲜' },
  { k: 'beauty', label: '美容' },
]);
const active = ref('food');
const products = ref([]);

function goProduct(p) { uni.navigateTo({ url: `/pages/product/detail?id=${p.id}&tenantId=${p.tenantId || ''}` }); }

watch(active, async () => { products.value = []; /* products.value = await listByCategory(active.value); */ });
onMounted(() => {
  const o = uni.getLaunchOptionsSync?.() || {};
  if (o.query?.k) active.value = o.query.k;
});
</script>

<style lang="scss" scoped>
@import '@/uni.scss';
.page { min-height: 100vh; background: $bg; display: flex; flex-direction: column; }
.layout { flex: 1; display: flex; overflow: hidden; }
.side { width: 92px; background: #F6F7F9; }
.side-item { padding: 14px 8px; text-align: center; font-size: 13px; color: $t2; }
.side-item.on { background: #fff; color: $o; font-weight: 800; }
.main { flex: 1; padding: 12px; }
.empty-wrap { padding-top: 40px; }
.prod { display: flex; gap: 10px; padding: 10px; background: #fff; border-radius: $r-md; margin-bottom: 8px; box-shadow: $sh-1; }
.prod-pic { width: 50px; height: 50px; border-radius: 10px; background: $o-50; color: $o; display: flex; align-items: center; justify-content: center; font-size: 22px; }
.prod-body { flex: 1; }
.prod-name { font-size: 13px; font-weight: 700; color: $t1; }
.prod-price { font-size: 14px; color: $o; font-weight: 800; margin-top: 4px; }
</style>
