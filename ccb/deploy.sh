...
if [[ -f "${ENV_FILE}" ]]; then
  # 强制清掉 .env 行尾 \r：Windows/notepad 编辑过的 .env 上传后会带 CRLF，
  # source 时 SK 末尾会多一个 \r 字节 → HMAC-SHA256 签名偏一字节 →
  # 火山即梦 / TOS 报 SignatureDoesNotMatch (CodeN=100010) → 视频生成 401。
  # 用户截图反馈反复 401 真因就是这个，root cause 修一次到位
  if grep -lq $'\r' "${ENV_FILE}" 2>/dev/null; then
    echo "[!] ${ENV_FILE} 含 CRLF（Windows 行尾）→ 自动转 LF（修复签名 401）" >&2
    sed -i 's/\r$//' "${ENV_FILE}"
  fi
...
# application.yaml 用 ${SMS_DEMO_MODE} / ${SMS_DEMO_CODE}（无 YUDAO_ 前缀）解占位符；
# 留 YUDAO_SMS_* 兼容老脚本，同时写无前缀名兜底，确保两套命名都有效。
SMS_DEMO_MODE=${YUDAO_SMS_DEMO_MODE:-true}
SMS_DEMO_CODE=${YUDAO_SMS_DEMO_CODE:-888888}
...
ENV_EOF
  # 同 sidecar/.env 修复：清掉行尾 \r 防止 SK 被 CRLF 污染导致签名 401
  sed -i 's/\r$//' "${ENV_UNIT}" 2>/dev/null || true
...
SIDECAR_ENV_EOF
  # 强制清掉行尾 \r（CRLF→LF）：Windows 编辑过的 .env 上传后会带 \r，
  # SK 末尾多一字节会让 HMAC-SHA256 签名偏，触发火山即梦 SignatureDoesNotMatch
  # 主 .env 在 deploy.sh 顶部 source 之前已经 sed 清过；这里再保险一次
  sed -i 's/\r$//' "${SIDECAR_DEST}/.env" 2>/dev/null || true
  umask 022
  ...
  # BGM 素材（可选）：sidecar /video/mux 按 bgmStyle 从此目录挑选混入。
  # 空目录时降级"仅 TTS"，不影响视频生成主流程。
  #
  # 注：download-bgm.sh 里的 Pixabay CDN URL 会随时间失效（CDN hash 改）；
  # 不再自动跑下载（避免一键部署被卡），改成在部署完毕后给运维一次明确指引。
  # 想要 BGM 的运维：cd ${SIDECAR_DEST}/bgm && bash download-bgm.sh
  # （或者把任何 mp3 重命名成 <style>_<n>.mp3 直接放进去，命名见 bgm/README.md）
  if [[ -d "${SIDECAR_DEST}/bgm" ]]; then
    local bgm_count
    bgm_count=$(ls "${SIDECAR_DEST}/bgm"/*.mp3 2>/dev/null | wc -l)
    if [[ "${bgm_count}" -lt 1 ]]; then
      info "BGM 目录暂为空 — 视频会自动降级'仅 TTS'（不影响主流程）"
      info "  想要 BGM 请手动跑：cd ${SIDECAR_DEST}/bgm && bash download-bgm.sh"
      info "  或自备 mp3 重命名为 <style>_<n>.mp3 放入此目录，style 见 bgm/README.md"
    else
      info "BGM 素材库 ${bgm_count} 首 mp3 已就绪"
    fi
  fi
